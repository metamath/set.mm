...
$(
#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#
                  NBG Set Theory without Equality
#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#
$)

  $( Declare the new constants and variables $)
  $c var = e. notsb C_ V , { } $.
  $v x y z w t u v $.
...
$)
  ...
  $( Define the subclass predicate $)
  df-sc1 $a |- ( x C_ y -> ( z e. x -> z e. y ) ) $.
  df-sc2 $a |- ( ( notsb ( x , y ) e. x -> notsb ( x , y ) e. y ) -> 
  	       	 x C_ y ) $.

  $( Substitution axiom for notsb $)
  ax-nseq1 $a |- ( x = y -> ( notsb ( x , z ) = notsb ( y , z ) ) ) $.

  $( Substitution axiom for notsb $)
  ax-nseq2 $a |- ( x = y -> ( notsb ( z , x ) = notsb ( z , y ) ) ) $.

  $( Substitution axiom for elementhood $)
  ax-eleq1 $a |- ( x = y -> ( x e. z -> y e. z ) ) $.

  $( Extensionality $)
  ax-ext $a |- ( x = y <-> ( x C_ y /\ y C_ x ) ) $.

  $( Axiom of the universal class $)
  ax-v $a |- x C_ V $.

  $( Axiom of pairing, part 1 $)
  ax-pr1 $a |- ( z e. { x , y } <-> ( z e. V /\ ( z = x \/ z = y ) ) ) $.

  $( Axiom of pairing, part 2 $)
  ax-pr2 $a |- { x , y } e. V $.

$(
#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#
                  Theorems from group A
#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#
$)

  $( Declare the constants for ordered pairs and singletons $)
  $c <. >. $.
  ...
  $( Biconditional for subclass $)
  scbi $p |- ( x C_ y <-> ( notsb ( x , y ) e. x -> notsb ( x , y ) e. y ) ) $=
    ( wsb vnsb wel wi df-sc1 df-sc2 impbii ) ABCABDZAEJBEFABJGABHI $.
    $( [?] $) $( [25-Apr-2011] $)

  $( Subclass obeys identity $)
  scid $p |- x C_ x $=
    ( vnsb wel wi wsb id df-sc2 ax-mp ) AABACZIDAAEIFAAGH $.
    $( [?] $) $( [25-Apr-2011] $)

  $( Subclass obeys trichotomy $)
  sctr $p |- ( ( x C_ y /\ y C_ z ) -> x C_ z ) $=
    ( wsb wa vnsb wel wi df-sc1 anim12i pm3.33 df-sc2 3syl ) ABDZBCDZEACFZAGZPB
    GZHZRPCGZHZEQTHACDNSOUAABPIBCPIJQRTKACLM $.
    $( [?] $) $( [25-Apr-2011] $)

  $( Equality obeys identity $)
  eqid $p |- x = x $=
    ( weq wsb ax-ext scid mpbir2an ) AABAACZGAADAEZHF $.
    $( [?] $) $( [25-Apr-2011] $)

  $( Equality is symmetric $)
  eqcom $p |- ( x = y <-> y = x ) $=
    ( wsb wa weq ancom ax-ext 3bitr4i ) ABCZBACZDJIDABEBAEIJFABGBAGH $.
    $( [?] $) $( [25-Apr-2011] $)

  ${
     eqcoms.1 $e |- ( x = y -> ph ) $.

     $( An inference form of ~ eqcom $)
     eqcoms $p |- ( y = x -> ph ) $=
       ( weq eqcom sylbi ) CBEBCEACBFDG $.
       $( [?] $) $( [25-Apr-2011] $)
  $}

  $( Equality is transtitive $)
  eqtr $p |- ( ( x = y /\ y = z ) -> x = z ) $=
    ( wsb wa weq sctr ancoms anim12i an4s ax-ext anbi12i 3imtr4i ) ABDZBADZEZBC
    DZCBDZEZEACDZCADZEZABFZBCFZEACFNQORUBNQETOREUAABCGROUACBAGHIJUCPUDSABKBCKLA
    CKM $.
    $( [?] $) $( [25-Apr-2011] $)

  $( An equivalence law for elementhood $)
  eleq1 $p |- ( x = y -> ( x e. z <-> y e. z ) ) $=
    ( weq wel ax-eleq1 wi eqcoms impbid ) ABDACEZBCEZABCFKJGBABACFHI $.
    $( [?] $) $( [25-Apr-2011] $)

  $( An equivalence law for elementhood $)
  eleq2 $p |- ( x = y -> ( z e. x <-> z e. y ) ) $=
    ( weq wsb wa wel wb ax-ext wi df-sc1 adantr adantl impbid sylbi ) ABDABEZBA
    EZFZCAGZCBGZHABIRSTPSTJQABCKLQTSJPBACKMNO $.
    $( [?] $) $( [25-Apr-2011] $)
  ...
  ${
     ...
     eleq1i $p |- ( x e. z <-> y e. z ) $=
       ( weq wel wb eleq1 ax-mp ) ABEACFBCFGDABCHI $.
       $( [?] $) $( [25-Apr-2011] $)

     $( Inference from equality to equivalence of membership $)
     eleq2i $p |- ( z e. x <-> z e. y ) $=
       ( weq wel wb eleq2 ax-mp ) ABECAFCBFGDABCHI $.
       $( [?] $) $( [25-Apr-2011] $)

     $( Inference from equality to equivalence of equality $)
     eqeq1i $p |- ( x = z <-> y = z ) $=
       ( weq wb eqeq1 ax-mp ) ABEACEBCEFDABCGH $.
       $( [?] $) $( [25-Apr-2011] $)

     $( Inference from equality to equivalence of equality $)
     eqeq2i $p |- ( z = x <-> z = y ) $=
       ( weq wb eqeq2 ax-mp ) ABECAECBEFDABCGH $.
       $( [?] $) $( [25-Apr-2011] $)
  $}

  $( If ` x ` is an element of anything, then it's an element of ` V ` $)
  elv $p |- ( x e. y -> x e. V ) $=
    ( vV wsb wel wi ax-v df-sc1 ax-mp ) BCDABEACEFBGBCAHI $.
    $( [?] $) $( [25-Apr-2011] $)

  $( Membership in an unordered pair $)
  elpr $p |- ( z e. { x , y } -> ( z = x \/ z = y ) ) $=
    ( vpr wel vV weq wo ax-pr1 pm3.27bi ) CABDECFECAGCBGHABCIJ $.
    $( [?] $) $( [25-Apr-2011] $)

  $( Identity law for unordered pairs $)
  prid1 $p |- ( x e. V -> x e. { x , y } ) $=
    ( vV wel weq wo vpr eqid orci wa ax-pr1 biimpri mpan2 ) ACDZAAEZABEZFZAABGD
    ZOPAHIRNQJABAKLM $.
    $( [?] $) $( [25-Apr-2011] $)

  $( Identity law for unordered pairs $)
  prid2 $p |- ( y e. V -> y e. { x , y } ) $=
    ( vV wel weq wo vpr eqid olci wa ax-pr1 biimpri mpan2 ) BCDZBAEZBBEZFZBABGD
    ZPOBHIRNQJABBKLM $.
    $( [?] $) $( [25-Apr-2011] $)

  $( Lemma for ~ prcom $)
  prcomlem $p |- { x , y } C_ { y , x } $=
    ( vpr wsb vnsb wel wi vV weq wo wa pm1.4 anim2i ax-pr1 3imtr4i scbi mpbir 
    ) ABCZBACZDRSEZRFZTSFZGTHFZTAIZTBIZJZKUCUEUDJZKUAUBUFUGUCUDUELMABTNBATNORSP
    Q $.
    $( [?] $) $( [25-Apr-2011] $)

  $( Unordered pairs commute $)
  prcom $p |- { x , y } = { y , x } $=
    ( vpr weq wsb ax-ext prcomlem mpbir2an ) ABCZBACZDIJEJIEIJFABGBAGH $.
    $( [?] $) $( [25-Apr-2011] $)
