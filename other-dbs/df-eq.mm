...
$(
#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#
                    Predicate Calculus with Defined Equality
#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#
$)

  $( Declare the new constants and variables for PC+DE $)
  $c A. = e. var $.
  $v x y z w t u v $.
...
$(
=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
          Pure predicate calculus axioms
=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
$)

  $( Quantified implication $)
  ax-qim $a |- ( A. x ( ph -> ps ) -> ( A. x ph -> A. x ps ) ) $.

  $( Quantified negation $)
  ax-qne $a |- ( -. A. x ph -> A. x -. A. x ph ) $.

  $( Quantifier commutation $)
  ax-qqcom $a |- ( A. x A. y ph -> A. y A. x ph ) $.
...
$(
=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
          Initial pure predicate calc theorems
=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
$)
  ...
  ${
     ...
     $( MP combined with generalization $)
     mpg $p |- ps $=
  ...
  ${
    ...
    alcoms $p |- ( A. y A. x ph -> ps ) $=
      ( wal ax-qqcom syl ) ACFDFADFCFBADCGEH $.
      $( [5-Aug-1993] $)
  $}
  ...
  ${
     ...
     alimi $p |- ( A. x ph -> A. x ps ) $=
       ( wi wal ax-qim mpg ) ABEACFBCFECABCGDH $.
       $( [?] $) $( [3-Oct-2011] $)
  ...
  ${
    ...
    al2imi $p |- ( A. x ph -> ( A. x ps -> A. x ch ) ) $=
      ( wal wi alimi ax-qim syl ) ADFBCGZDFBDFCDFGAKDEHBCDIJ $.
      $( [5-Aug-1993] $)
  $}

  ${
    alanimi.1 $e |- ( ( ph /\ ps ) -> ch ) $.
    $( Variant of ~ al2imi with conjunctive antecedent.  (Contributed by
  ...
  $( Distribute quantifier over and connective $)
  alan $p |- ( A. x ( ph /\ ps ) <-> ( A. x ph /\ A. x ps ) ) $=
    ( wa wal pm3.26 alimi pm3.27 jca id alanimi impbii ) ABDZCEZACEZBCEZDNOPMAC
    ABFGMBCABHGIABMCMJKL $.
    $( [?] $) $( [3-Oct-2011] $)

  $( Distribute quantifier over biconditional $)
  ...
  ${
    ...
    $( An axiom scheme of standard predicate calculus $)
    stdpc5 $p |- ( A. x ( ph -> ps ) -> ( ph -> A. x ps ) ) $=
      ( wi wal ax-qim syl5 ) ABECFACFBCFAABCGDH $.
      $( [?] $) $( [3-Oct-2011] $)
  $}

$(
=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
          Equality and initial consequences
=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
$)

  ${ $d x z $. $d y z $.
     $( Define equality $)
     df-eq $a |- ( x = y <-> ( A. z ( z e. x <-> z e. y ) /\
     	      	       	       A. z ( x e. z <-> y e. z ) ) ) $.
  $}
  ...
  ${ $d x y $.
     ...
     eqid $p |- x = x $=
       ( vy weq wel wb wal df-eq biid ax-gen mpbir2an ) AACBADZKEZBFABDZMEZBFAA
       BGLBKHINBMHIJ $.
       $( [?] $) $( [3-Oct-2011] $)
  $}
  ...
  ${ $d x z $. $d y z $.
     $( Equality commutes $)
     eqcom $p |- ( x = y <-> y = x ) $=
       ( vz wel wb wal wa weq bicom albii anbi12i df-eq 3bitr4i ) CADZCBDZEZCFZ
       ACDZBCDZEZCFZGONEZCFZSREZCFZGABHBAHQUCUAUEPUBCNOIJTUDCRSIJKABCLBACLM $.
       $( [?] $) $( [3-Oct-2011] $)
  $}
  ...
  ${
     ...
     $( An inference commuting equality in antecedent $)
     eqcoms $p |- ( y = x -> ph ) $=
       ( weq eqcom sylbi ) CBEBCEACBFDG $.
       $( [?] $) $( [3-Oct-2011] $)
  $}

  ${ $d x w $. $d y w $. $d z w $.
     $( Equivalence law for equality $)
     eqeq1 $p |- ( x = y -> ( x = z <-> y = z ) ) $=
       ( vw weq wel wb wal wa df-eq bibi1 alimi albi syl bi2anan9 sylbi 
       3bitr4g ) ABEZDAFZDCFZGZDHZADFZCDFZGZDHZIZDBFZTGZDHZBDFZUDGZDHZIZACEBCER
       SUHGZDHZUCUKGZDHZIUGUNGABDJUPUBUJURUFUMUPUAUIGZDHUBUJGUOUSDSUHTKLUAUIDMN
       URUEULGZDHUFUMGUQUTDUCUKUDKLUEULDMNOPACDJBCDJQ $.
       $( [?] $) $( [3-Oct-2011] $)
  $}

   $( Equivalence law for equality $)
   eqeq2 $p |- ( x = y -> ( z = x <-> z = y ) ) $=
     ( weq eqeq1 eqcom 3bitr4g ) ABDACDBCDCADCBDABCECAFCBFG $.
     $( [?] $) $( [3-Oct-2011] $)
